...
for i, a in enumerate(ax.flat):
    ...
    a.set_title(f'Participant {i+1} - {participant_info["info_names"][i]}')
    a.set_ylim([0.3, 0.8])
    a.set_xlim([-0.2, 0.8])
...
#     a.set_xticklabels('')
#     a.set_yticklabels('')
# fig.savefig(os.path.join(graph_dir,'Results','Things_Localizer_Decoding_TimeCourse_SingleParticipant.svg'), dpi=300)
...
#######################################
"""
Resting: Sequenceness Measure Empricial Transition Matrices
...
#######################################
"""
Resting: Sequenceness Measure by Participant
...
#######################################
"""
Resting: Sequenceness Measure Supplemental
...
#######################################
"""
Resting: Sequenceness Replay Examples
...
axes[0].bar(bins[1:], unity_density, width=widths, alpha=0.7, edgecolor='black')
axes[0].axhline(1/len(bins), color='red', linestyle='--', label='Uniform expectation')
...
axes[1].scatter(theoretical_quantiles, sorted_times, alpha=0.5, s=10)
