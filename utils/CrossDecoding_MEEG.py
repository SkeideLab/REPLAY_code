# -*- coding: utf-8 -*-
"""
Cross-Decoding Class

Created on Thu May 29 00:33:46 2025
...
class CrossDecoding_MEEG(BaseEstimator, ClassifierMixin):
    ...
    multi_model : bool, default=False
        If True, train separate binary classifier for each class.
        If False, train single multi-class classifier.
    ...
    def _validate_input(self, X, y=None):
        ...
        if X.ndim != 3:
            raise ValueError(
                f"X must be 3D array (n_samples, n_features, n_times), got {X.ndim}D"
            )
        ...
        if y is not None:
            ...
            if len(y) != X.shape[0]:
                raise ValueError(
                    f"X and y have incompatible shapes: {X.shape[0]} vs {len(y)}"
                )
    ...
    def _get_time_indices(self, n_times):
        ...
        if self.train_times is None:
            if self.training_timepoint is not None:
                raise ValueError(
                    "times must be provided when training_time is specified"
                )
        ...
        if len(times_array) != n_times:
            raise ValueError(
                f"Length of times ({len(times_array)}) must match last dimension of X ({n_times})"
            )
    ...
    def _prepare_training_data(self, X, y):
        ...
        if self.baseline_timepoint is not None:
            if self.train_times is None:
                raise ValueError(
                    "times must be provided when baseline_time is specified"
                )
            
            if isinstance(self.baseline_timepoint, list):
                min_idx = np.argmin(
                    np.abs(
                        np.array(self.train_times) - self.baseline_timepoint[0]
                    )
                )
                max_idx = np.argmin(
                    np.abs(
                        np.array(self.train_times) - self.baseline_timepoint[1]
                    )
                )
                n_draws = self.baseline_timepoint[2]
                
                ind_combs = np.array(list(product(range(min_idx, max_idx), range(n_samples))))
                
                # Select baseline trials (same number as original trials)
                rng = np.random.RandomState(self.random_state)
                baseline_trial_indices = rng.choice(len(ind_combs), n_draws*n_samples, replace=False)
                
                trl_inds = ind_combs[baseline_trial_indices,1]
                time_inds = ind_combs[baseline_trial_indices,0]
                
                baseline_X = X[trl_inds, :, time_inds]
                baseline_y = np.zeros(len(baseline_trial_indices), dtype=y.dtype)
                baseline_data = (baseline_X, baseline_y)
                
            else:
                baseline_idx = np.argmin(
                    np.abs(np.array(self.train_times) - self.baseline_timepoint)
                )
    ...
    def fit(self, X, y):
        ...
        if self.include_zero:
            ...
            self.classes_indices_ = np.arange(len(self.classes_))
        ...
        for i, train_time_idx in enumerate(
            trange(len(training_indices), desc=desc, disable=disable_tqdm)
        ):
            ...
            if self.multi_model:
                ...
                for class_label in self.classes_:
                    binary_y = (train_y == class_label).astype(int)
                    timepoint_decoders[class_label] = clone(self.base_estimator)
                    timepoint_decoders[class_label].fit(train_X, binary_y)
            else:
                # Train single multi-class classifier
                timepoint_decoders["singlemodel"] = clone(self.base_estimator)
                timepoint_decoders["singlemodel"].fit(train_X, train_y)
    ...
    def _get_prediction_method(self, method_name, train_time_idx):
        ...
        if self.multi_model:
            for class_label in self.classes_:
                decoder = timepoint_decoders[class_label]
                if hasattr(decoder, method_name):
                    methods[class_label] = getattr(decoder, method_name)
                else:
                    raise AttributeError(
                        f"Base estimator does not have {method_name} method"
                    )
        else:
            decoder = timepoint_decoders["singlemodel"]
            if hasattr(decoder, method_name):
                methods["singlemodel"] = getattr(decoder, method_name)
            else:
                raise AttributeError(
                    f"Base estimator does not have {method_name} method"
                )
    ...
    def _apply_prediction_method(self, X, method_name):
        ...
        if len(self.training_indices_) == 1:
            # Single training timepoint: output shape (n_samples, n_classes, n_times)
            if self.multi_model:
        ...
        for train_idx, train_time_idx in enumerate(
            trange(
                len(self.training_indices_),
            ...
            prediction_methods = self._get_prediction_method(
                method_name, actual_train_time_idx
            )
            ...
            for test_t in range(n_times):
                ...
                if self.multi_model:
                    # Apply each binary classifier multiple models
                    for i, class_label in enumerate(self.classes_):
                        method = prediction_methods[class_label]
                        ...
                        if len(self.training_indices_) == 1:
                            predictions[:, i, test_t] = pred_t
                        else:
                            predictions[:, i, train_idx, test_t] = pred_t
                else:
                    # Apply multi-class classifier in single model
                    method = prediction_methods["singlemodel"]
                    pred_t = method(X_t)
                    ...
                    if len(self.training_indices_) == 1:
                        ...
                        if method_name == "predict":
                            predictions[:, train_idx, test_t] = (
                                pred_t  # [self.classes_indices_]
                            )
                        elif (
                            method_name == "decision_function"
                            and len(self.classes_) == 2
                        ):
                            predictions[:, train_idx, test_t] = pred_t[
                                self.classes_indices_
                            ]
                        else:
                            predictions[:, :, train_idx, test_t] = pred_t[
                                :, self.classes_indices_
